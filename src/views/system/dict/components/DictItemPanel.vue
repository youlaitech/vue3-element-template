<template>
  <el-card ref="tableWrapperRef" class="page-content" shadow="never">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-3">
      ...
      <div class="flex items-center gap-2">
        <el-button v-hasPerm="['sys:dict-item:create']" type="primary" @click="handleCreateClick()">
          新增
        </el-button>
        <el-button
    ...
    <div class="page-table-wrapper">
      ...
      >
        ...
        <el-table-column label="字典标签" prop="label" show-overflow-tooltip />
        ...
        <el-table-column label="标签类型" align="center" width="120">
          <template #default="scope">
            ...
            <span v-else class="dict-item-panel__plain">文本</span>
          </template>
        </el-table-column>
...
>({
  ...
  request: (query) => DictAPI.getDictItemPage(props.dictCode, query),
});
...
async function handleEditClick(row: DictItem): Promise<void> {
  ...
  const data = await DictAPI.getDictItemFormData(props.dictCode, row.id);
  Object.assign(formData, data);
  openDialog();
}

/**
 * 字典项变更后清除本地缓存，避免 DictSelect/DictTag 沿用旧数据直到 SSE 推送到达
 */
function invalidateDictCache(): void {
  dictStore.removeDictItem(props.dictCode);
}
...
async function handleSubmit(): Promise<void> {
  ...
  try {
    if (id) {
      await DictAPI.updateDictItem(props.dictCode, id, formData);
      ElMessage.success("修改成功");
    } else {
      await DictAPI.createDictItem(props.dictCode, formData);
...
async function handleDelete(id?: string): Promise<void> {
  ...
  loading.value = true;
  try {
    await DictAPI.deleteDictItems(props.dictCode, itemIds);
